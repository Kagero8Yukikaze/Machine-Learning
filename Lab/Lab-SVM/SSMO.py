...
class SSMO_optimizer():
    ...
    def fit(
        ...
        while passes < max_passes:
            # the number of alpha variables that have been changed in this pass, 
            #   when num_changed_alphas == 0, the training process should be terminated.
    ...
    def compute_L_H(
        ...
        # alpha[i]_old * y[i] + alpha[j]_old * y[j] = alpha[i]_new * y[i] + alpha[j]_new * y[j]
        if is_yi_equals_yj:
            # 0 <= alpha[i]_new = alpha[i]_old + alpha[j]_old - alpha[j]_new <= C
            # alpha[i]_old + alpha[j]_old - C <= alpha[j]_new <= alpha[i]_old + alpha[j]_old
            H = min(self.C, self.alpha[j] + self.alpha[i])
            L = max(0, self.alpha[j] + self.alpha[i] - self.C)
        else:
            # 0 <= alpha[i]_new = alpha[j]_new - (alpha[j]_old - alpha[i]_old) <= C
            # alpha[j]_old - alpha[i]_old <= alpha[j]_new <= C + alpha[j]_old - alpha[i]_old
    ...
    def compute_eta(self, xi, xj):
        '''
        Compute eta, which is the second derivative of the objective function of dual form of SVM (The equivalent minimization problem).
    ...
    def compute_new_aj(self, x_train, y_train, i, j, eta, L, H):
        ...
        if eta > 0:
            ...
            aj_opt = self.alpha[j] + y_train[j] * (Ei - Ej) / eta
