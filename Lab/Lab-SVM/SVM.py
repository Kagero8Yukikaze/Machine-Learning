...
class SVM():
    ...
    def predict(
        ...
        # Asumme that: self.b, self.alpha, self.support_labels, self.support_vectors and self.kernel_fn 
        #   are already given (They will be overwritten by SSMO optimization, which will be implemented in the next part)

        # f(x) = sum(alpha_i * y_i * k(x_i,x)) + b
        # k : (n, n_support)
        # alpha * label : (n_support, )
        scores = np.dot(self.kernel_fn(x, self.support_vectors), self.alpha * self.support_labels) + self.b
