...
class LogisticRegression():
    ...
    def predict(
        ...
        # no need for bias because we have already added bias in fit?
    ...
    def fit(
        ...
        self.w = np.random.normal(0, 1, x.shape[1]) # random initialize w
        loss_history = []
        w_module_history = []
    ...
    def update(
        ...
        partial = (y.reshape(-1,1) * x) / (1 + np.exp(y * z).reshape(-1,1))
